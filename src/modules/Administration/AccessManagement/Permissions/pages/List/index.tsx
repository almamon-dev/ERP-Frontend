...
export default function PermissionList() {
    const [data, setData] = useState([
        { id: 1, name: 'User Management Read/Write', code: 'PERM_USERS_ALL', status: 'Active', createdAt: '2026-07-10' },
        { id: 2, name: 'Role Assignment', code: 'PERM_ROLES_ASSIGN', status: 'Active', createdAt: '2026-07-12' },
        { id: 3, name: 'Audit Logs View', code: 'PERM_AUDIT_VIEW', status: 'Inactive', createdAt: '2026-07-15' },
    ]);
    ...
    const handleBulkDelete = (ids: number[]) => {
        if (confirm(`Are you sure you want to delete ${ids.length} permissions?`)) {
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Permission Name', render: (item) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'code', label: 'Permission Code', render: (item) => <span className="font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded text-[11.5px]">{item.code}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
        ...
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. User Management Read/Write" className="h-8 text-[12.5px]" />
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.code : ''} placeholder="e.g. PERM_USERS_ALL" className="font-mono h-8 text-[12.5px]" />
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Textarea defaultValue={isEdit ? editItem?.description : ''} placeholder="Describe permission scope..." className="min-h-[60px] text-[12.5px] py-1.5" />
                </div>
            </div>
            ...
            <div className="p-2.5 px-3 bg-slate-50 border border-slate-200/70 rounded-md flex items-center justify-between mt-1">
                <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">Is this permission active in the system?</p>
                </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Permissions</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Manage system permissions and access levels.</p>
                </div>
                <Button 
