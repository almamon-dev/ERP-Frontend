...
export default function AboutMePage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'field', label: 'Field Label', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.field}</span> },
        { id: 'val', label: 'Personal Value', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.val}</span> },
        { id: 'verification', label: 'Verification', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.verification}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Field Label</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.field : ''} placeholder="e.g. Blood Group" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Personal Value</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.val : ''} placeholder="e.g. O+" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Verification</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.verification : ''} placeholder="e.g. Verified by HR" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">About Me (Employee Profile)</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Personal information, emergency contact details, educational background, and employment info.</p>
                </div>
                <Button 
