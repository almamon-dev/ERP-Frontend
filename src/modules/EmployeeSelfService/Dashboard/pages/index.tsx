...
export default function EmployeeDashboardPage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "metric": "Monthly Attendance Rate",
                "val": "96.8%",
                "remarks": "Excellent",
                "status": "Active",
    ...
    const columns: Column[] = [
        ...
        { id: 'metric', label: 'Metric', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.metric}</span> },
        { id: 'val', label: 'Value', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.val}</span> },
        { id: 'remarks', label: 'Remarks', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.remarks}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Metric</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.metric : ''} placeholder="e.g. Total Present Days" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Value</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.val : ''} placeholder="e.g. 22 Days" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Remarks</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.remarks : ''} placeholder="e.g. On Track" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Employee Dashboard</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Personal attendance summary, leave balances, pending requests, and announcements.</p>
                </div>
                <Button 
