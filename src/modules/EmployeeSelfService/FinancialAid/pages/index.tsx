...
export default function LoanFinancialAidPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'loanType', label: 'Loan Type', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.loanType}</span> },
        { id: 'principal', label: 'Principal Amount', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.principal}</span> },
        { id: 'emi', label: 'Monthly Deduction', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.emi}</span> },
        { id: 'tenure', label: 'Tenure (Months)', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.tenure}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Loan Type</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.loanType : ''} placeholder="e.g. Salary Advance / Emergency Loan" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Principal Amount</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.principal : ''} placeholder="e.g. ৳ 50,000" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Monthly Deduction</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.emi : ''} placeholder="e.g. ৳ 5,000 / month" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Tenure (Months)</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.tenure : ''} placeholder="e.g. 10 Months" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Loan & Financial Aid</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Apply for company salary loans, medical aid, and monthly installment schedules.</p>
                </div>
                <Button 
