...
export default function IOUPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'refNo', label: 'IOU Reference', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.refNo}</span> },
        { id: 'amount', label: 'Requested Amount', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.amount}</span> },
        { id: 'purpose', label: 'Purpose', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.purpose}</span> },
        { id: 'settleDate', label: 'Settlement Target', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.settleDate}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">IOU Reference</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.refNo : ''} placeholder="e.g. IOU-2026-042" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Requested Amount</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.amount : ''} placeholder="e.g. ৳ 15,000" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Purpose</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.purpose : ''} placeholder="e.g. Team Workshop Expenses" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Settlement Target</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.settleDate : ''} placeholder="e.g. 2026-08-05" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">IOU (I Owe You / Advance Cash)</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Apply for temporary cash advances for official expenses and track settlements.</p>
                </div>
                <Button 
            ...
            <DataTable 
                data={filteredData} 
                columns={columns}
                searchPlaceholder="Search IOU requisitions..."
