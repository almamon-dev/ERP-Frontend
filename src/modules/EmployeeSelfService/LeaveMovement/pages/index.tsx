...
export default function LeaveAndMovementPage() {
    const [data, setData] = useState<any[]>([
        ...
        {
                "id": 2,
                "type": "Official Movement",
                "startTime": "2026-07-20 11:00 AM",
                "endTime": "2026-07-20 03:00 PM",
                "reason": "Bank & Tax Work",
                "status": "Approved",
    ...
    const columns: Column[] = [
        ...
        { id: 'type', label: 'Application Type', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.type}</span> },
        { id: 'startTime', label: 'Start Date / Time', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.startTime}</span> },
        { id: 'endTime', label: 'End Date / Time', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.endTime}</span> },
        { id: 'reason', label: 'Reason / Location', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.reason}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Application Type</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.type : ''} placeholder="e.g. Official Outdoor Movement" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Start Date / Time</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.startTime : ''} placeholder="e.g. 2026-07-28 10:00 AM" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">End Date / Time</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.endTime : ''} placeholder="e.g. 2026-07-28 02:00 PM" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Reason / Location</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.reason : ''} placeholder="e.g. Client Site Visit" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Leave And Movement</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Apply for short leave, official outdoor movement, and annual leave requests.</p>
                </div>
                <Button 
