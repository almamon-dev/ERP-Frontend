...
export default function SupervisorReportingStructurePage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "supName": "David Miller",
                "designation": "VP of Engineering",
                "email": "david@company.com",
                "level": "Primary Supervisor (L1)",
                "status": "Active",
                "createdAt": "2026-01-01"
        },
        {
                "id": 2,
                "supName": "Emily Watson",
                "designation": "HR Director",
                "email": "emily@company.com",
                "level": "HR Approver (L2)",
                "status": "Active",
                "createdAt": "2026-01-01"
        }
]);
    ...
    const columns: Column[] = [
        ...
        { id: 'supName', label: 'Supervisor Name', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.supName}</span> },
        ...
        { id: 'level', label: 'Approval Level', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.level}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Supervisor Name</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.supName : ''} placeholder="e.g. David Miller" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.designation : ''} placeholder="e.g. VP of Engineering" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.email : ''} placeholder="e.g. david@company.com" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Approval Level</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.level : ''} placeholder="e.g. Level-1 Approver" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Supervisor & Reporting Structure</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">View immediate reporting manager, line supervisor, and team organizational chart.</p>
                </div>
                <Button 
