...
export default function TimeManagementPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'logDate', label: 'Date', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.logDate}</span> },
        { id: 'clockIn', label: 'Clock In', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.clockIn}</span> },
        { id: 'clockOut', label: 'Clock Out', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.clockOut}</span> },
        { id: 'hours', label: 'Working Hours', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.hours}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Date</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.logDate : ''} placeholder="e.g. 2026-07-26" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Clock In</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.clockIn : ''} placeholder="e.g. 08:55 AM" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Clock Out</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.clockOut : ''} placeholder="e.g. 05:15 PM" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Working Hours</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.hours : ''} placeholder="e.g. 8.2 Hours" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Time Management</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Daily check-in / check-out history, attendance regularizations, and roster schedules.</p>
                </div>
                <Button 
